 ...
 */
public class ClassPoints extends Object3D
{
    ...
    public ClassPoints(int numberOfPoints, int floatsPerPoint, boolean isCreateColors)
    {
        super();
    ...
    public void updatePoints(int pointCount, FloatBuffer pointCloudBuffer)
    {
        ...
        mGeometry.changeBufferData(mGeometry.getVertexBufferInfo(), mGeometry.getVertices(), 0, pointCount * mFloatsPerPoint);
    }
    ...
    public void updatePoints(int pointCount, FloatBuffer points, float[] colors)
    {
        ...
        {
            throw new RuntimeException(String.format("pointClount = %d exceeds maximum number of points = %d", pointCount, mMaxNumberOfVertices));
        }
        // mGeometry.setNumIndices(pointCount);
        mGeometry.setVertices(points);
        mGeometry.changeBufferData(mGeometry.getVertexBufferInfo(), mGeometry.getVertices(), 0, pointCount * mFloatsPerPoint);
        mGeometry.setColors(colors);
        mGeometry.changeBufferData(mGeometry.getColorBufferInfo(), mGeometry.getColors(), 0, pointCount * mFloatsPerColor);
    }
